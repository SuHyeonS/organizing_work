

//훅 
//useState: 상태(state) 관리
//useEffect: 컴포넌트가 처음 실행되거나 특정 값이 바뀔 때 동작
//useCallback: 함수를 메모이제이션해서 불필요한 재렌더링 방지
import React, { useState, useEffect, useCallback, useRef } from "react";
import axios from "axios"; //서버와 통신할 때 HTTP 요청(GET, POST 등)을 보내는 라이브러리입니다.
import "./WorkModal.css"; // 모달 스타일


//export default: 다른 파일에서 이 컴포넌트를 불러 쓸 수 있게 내보냅니다. work: propsWork > work -> propsWork
export default function WorkModal({ fields, workPk, onClose, moveProps }) {

    //목록
    const [work, setWork] = useState(); // ✅ props로 받은 work를 상태로 복사
    const [workList, setWorkList] = useState(); 
    
    const [editMode, setEditMode] = useState(false); // 🔹 전체 수정 모드 여부
    const [newRows, setNewRows] = useState([]);// 새로 추가한 행들
    const [ModalLoading, setModalLoading] = useState(false); //로딩

    //하위 이동 기능
    const { handleCheck, moveSublist, saveCheckList } = moveProps; // ✅ 구조분해로 꺼냄
    
/*
    //검색 > 부모에게 이관!!
    //useCallback 불필요한 재렌더링 방지, ESLint 경고 방지
    //async/await axios 요청이 비동기라서, 응답이 올 때까지 기다리도록 함
    const setWorkList = useCallback(async () => {
    //const getWorkListDetail = async (workPk) => {

        console.log("work.workPk : ", work.workPk);
        try {
        setLoading(true); //로딩표시

        //서버호출
        //axios.get(`/api/work/${id}/children`) //subList
        const res = await axios.get(`/api/work/${work.workPk}/children`);
        console.log("data:", res.data);
        setWorkList(res.data); //받아온 데이터를 workList 상태에 저장
        return res; //결과 반환
        } catch (err) {
        console.error("검색 오류:", err);
        } finally {
        setLoading(false);
        }
    }, [work.workPk]); // 최신 검색어 반영

*/
    //자식목록
    async function getWorkList(key){
        console.log("모달 getWorkList key : ",key);
        if(!key){ key = workPk; } //최초조회시
        try {
                const res = await axios.get(`/api/work/${key}/children`);
                setWorkList(res.data); // 상태에 데이터 반영
                console.log("모달 setWorkList key : ",res.data);
            } catch (err) {
                console.error(err);
            } finally {
                //setModalLoading(false);
            }
    }

    //상세정보
    async function getWork(key){
        if(!key){ key = workPk; }
        try {
                const res = await axios.get(`/api/work/${key}`); //one
                setWork(res.data); // 상태에 데이터 반영
                console.log("getWork : ", res.data);
            } catch (err) {
                console.error(err);
            } finally {
                //setModalLoading(false);
            }
    }


    //모달 갱신
    const reLoad = async (key, check) => {
        
        if(!key){ key = workPk; } //최초 접속시 

        let finalKey=key;
        console.log("reLoad modal 초기 key:", finalKey, check);
        //check : up down
        try {
            setModalLoading(true);
            //이전 부모불러오기
            if(check === "up"){
                const res = await axios.get(`/api/work/${finalKey}/parentId`);
                console.log("Work 객체:", res.data);

                // parent가 존재하면 부모 PK 사용
                if (res.data && res.data.workPk) {
                //if (res.data && res.data.parent && res.data.parent.workPk) {
                    finalKey = res.data.workPk;
                } else {
                    console.warn("부모가 없습니다. 현재 id 유지:", finalKey);
                    alert("상위 게시물이 없습니다.");
                }

                //console.log("업데이트된 id:", id);
                
            }
            console.log("reLoad modal update Key : ", finalKey);
            /*
            //자식불러오기
            // 상세 + 하위 리스트를 동시에 요청
            const [detailRes, subListRes] = await Promise.all([
                axios.get(`/api/work/${finalKey}`), //one
                axios.get(`/api/work/${finalKey}/children`) //subList
            ]);
            setWork(detailRes.data);
            setWorkList(subListRes.data);
            */

            getWorkList(finalKey);
            getWork(finalKey);

        } catch (err) {
            console.error("상세 조회 오류:", err);
        } finally {
            setModalLoading(false);
        }
    };

    
    //최초 한번만 실행 > 목록출력
    useEffect(() => {
        reLoad(workPk,"down");
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, []);

    /*
    if (!work) {
    return <div>로딩 중...</div>;
    }
    */
    //{ModalLoading && <div className="loading-overlay">로딩 중...</div>}
        

    // 수정 중 값 변경
    const handleChange = (index, field, value) => {
        const updated = [...workList];
        updated[index][field] = value;
        setWorkList(updated);
    };


    // 새 행 추가
    const handleAddRow = () => {

        // 새 행 기본 구조 자동 생성
        //fields.reduce()는 fields 배열을 돌면서 누적 객체(acc) 를 만듬
        const emptyRow = fields.reduce((acc, f) => {
        acc[f.key] = ""; // 각 key에 빈 문자열 값 설정
        return acc;
        }, { parent: { workPk: work.workPk }}); // 초기값 셋팅

        setNewRows([ ...newRows,emptyRow ]);
    };


    // 새 행 값 변경
    //React의 상태(state)는 불변성(immutability) 을 유지해야 합니다. 즉, 기존 배열이나 객체를 직접 수정하면 안 되고, 항상 “복사본을 만들어서 수정 → 다시 setState()” 해야 합니다.
    //1️⃣ const updated = [...newRows] → 배열 복사
    //2️⃣ updated[index][field] = value → 해당 행의 특정 필드값만 수정
    //3️⃣ setNewRows(updated) → 새로운 배열로 상태 갱신
    const handleNewChange = (index, field, value) => {
        const updated = [...newRows];
        updated[index][field] = value;
        setNewRows(updated);
    };

    // 전체 저장
    const handleSaveAll = async () => {
        /*
        if(workList){
            for(var i=0; i < workList.length; i++){
                workList[i].parent = work.workPk;
            }
        }
        */

        const payload = {
            workPk:work.workPk, // 🔹 부모 workPk 전달 > parent 객체 구조로 전달
            updatedList: editMode ? workList : [], // 수정모드일 때만 보냄
            newList: newRows, // 신규 입력된 행
        };

        console.log("call.. handleSaveAll : ", payload);
        /*
        if(!editMode){ //수정모드 라면
        payload = payload.concat(workList); //payload = [...payload, ...workList];
        }
        */

        try {
            await axios.post("/api/work/saveAll", payload);

            alert("저장되었습니다!");
            setEditMode(null); //수정중인행
            setNewRows([]); 

            getWorkList(work.workPk); // 🔹 단순히 재조회만
        } catch (err) {
            console.error("저장 오류:", err);
        }
    };


    // ✅ 새 행 삭제 함수
    const handleDeleteNewRow = (index) => {
    //배열에서 특정 인덱스(index)의 요소를 제외한 나머지만 남기는 코드
    //filter 콜백의 두 번째 매개변수 i는 인덱스(index)이고, _는 현재 값인데 안 쓸 때 _로 표시하는 관례예요
    const updated = newRows.filter((_, i) => i !== index);  //filter false 면 남김
    setNewRows(updated);
    };


    // DB 데이터 삭제
    //행위	HTTP 메서드	URL 예시	의미
    //조회	GET	/api/work/1	id가 1인 Work 조회
    //등록	POST	/api/work	새로운 Work 생성
    //수정	PUT / PATCH	/api/work/1	id가 1인 Work 수정
    //삭제	DELETE	/api/work/1	id가 1인 Work 삭제
    const workListDelete = async (id) => {
    if (!window.confirm("정말 삭제하시겠습니까?")) return;

    try {
        await axios.delete(`/api/work/${id}`); // DELETE 요청으로 ID 전달
        alert("삭제되었습니다!");
        getWorkList(work.workPk); // 리스트 다시 불러오기
    } catch (err) {
        console.error("삭제 오류:", err);
        alert("삭제 중 오류가 발생했습니다.");
    }
    };



    // 단건 수정
    const workSave = async () => {
        console.log("수정 : ",work);
        var id = work.workPk;
    try {
        await axios.put(`/api/work/${id}`, work);
        alert("수정되었습니다!");
    } catch (err) {
        console.error("저장 오류:", err);
    }
    };
    
    //상위목록으로 이관
    const moveTopList = async (id) => {
        console.log("moveTopList : ",work);

        if (!window.confirm("해당목록의 상위로 이동하시겠습니까? ")) return;

        if (!work || !work.workPk) {
            alert("상위 부모 정보가 없습니다. 이동할 수 없습니다.");
            return;
        }
        
        const payload = {
            subList: [{ workPk: id }],  // 자식이되는키
            //workPk: work.workPk, // 부모가 되는키
        };
        console.log("moveTopList 부모가 되는 키 : ", work.workPk, "자녀가 되는 키 : ", id);

        try {
        var mge = await axios.put(`/api/work/${work.workPk}/moveParent`, payload);
        alert(mge.data);
        await getWorkList(work.workPk); // 리스트 다시 불러오기
        } catch (err) {
        console.error("이관 오류:", err);
        alert("이관 중 오류가 발생했습니다.");
        }
        
    };

    if (!work) return null; // work가 없으면 렌더링 안 함

    //if (ModalLoading) return <div>로딩 중...</div>;

    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                {/* 좌측 */}
                <div className="modal-left">
                    <h2>상세 정보</h2>
                    <p onClick={() => reLoad(work.workPk,"up")} style={{ cursor: "pointer", color: "blue" }}>뒤로가기</p>

                    <table className="work-table" style={{ width: "90%" }}>
                        <thead>
                            {fields && fields.length > 0 && fields.map((f) => (
                            <tr key={f.key}>
                                <th style={{background: "#dfe6e9", fontWeight: "500"}}>{f.label}</th>
                                <th style={{}} key={f.key}>
                                    <input 
                                        type={f.type} 
                                        value={work[f.key] || ""}
                                        //readOnly //읽기전용
                                        onChange={(e) =>
                                            setWork({
                                                ...work, //스프레드 연산자(...work)**로 복사.
                                                [f.key]: e.target.value, // 입력값을 상태에 반영
                                            })}
                                    ></input>
                                </th>
                            </tr>
                            ))}
                        </thead>
                    </table>
                    <button style={{margin:"5px"}} onClick={workSave}>수정</button>
                    <button onClick={onClose}>닫기</button>
                </div>

                {/* 우측 */}
                <div className="modal-right">
                        
                        <div style={{ margin: "10px 0px 10px 0px" }}>
                            <button onClick={handleAddRow}>+ 행 추가</button>
                            <button onClick={() => setEditMode(!editMode)} style={{ marginLeft: "8px" }}>
                            ✏️ {editMode ? "보기모드" : "수정모드"}
                            </button>
                            <button onClick={handleSaveAll} style={{ marginLeft: "8px" }}>
                            💾 저장
                            </button>
                        </div>

                        {/* 테이블 컨테이너 */}
                        <table className="work-table" style={{ width: `${7*(fields.length+4)}%` }}>
                            <thead>
                            <tr style={{ background: "#dfe6e9", fontWeight: "bold"}}>
                                {/* 컬럼 헤더 */}
                                <th>하위</th>
                                {fields && fields.length > 0 && fields.map((f) => (
                                <th style={{ width: `${100 / (fields.length+5)}%` }} key={f.key}>{f.label}</th> //체크,상세,하위,삭제
                                ))}
                                <th style={{ width: `${100 / (fields.length+4)}%` }}>상세목록</th>
                                <th style={{ width: `${100 / (fields.length+4)}%` }}>하위이동</th>
                                <th style={{ width: `${100 / (fields.length+4)}%` }}>상위이동</th>
                                <th style={{ width: `${100 / (fields.length+4)}%` }}>삭제</th>
                            </tr>
                            </thead>
                            
                            <tbody>
                            {/* 기존 목록 */}
                            {workList.length > 0 && fields.length > 0 ? (
                                workList.map((work, index) => (
                                <tr key={work.workPk}>
                                    <td>
                                        <input type="checkbox" className="input-check" onClick={() => handleCheck(work)}/>
                                    </td>
                                    {fields.map((f) => (
                                        <td key={f.key}>
                                        {editMode ? (
                                            <input
                                            type={f.type}
                                            className="input-box"
                                            value={work[f.key] || ""}
                                            onChange={(e) => handleChange(index, f.key, e.target.value)}
                                            />
                                        ) : (
                                            work[f.key] || "-"
                                        )}
                                        </td>
                                    ))}
                                    <td onClick={() => reLoad(work.workPk, "down")} style={{ cursor: "pointer", color: "blue" }}>상세</td>
                                    <td>
                                        <button style={{ color: "blue" }} onClick={() => moveSublist(work.workPk)}>하위이동</button>
                                    </td>
                                    <td>
                                        <button style={{ color: "blue" }} onClick={() => moveTopList(work.workPk)}>상위이동</button>
                                    </td>
                                    <td>
                                        <button style={{ color: "red" }} onClick={() => workListDelete(work.workPk)}>삭제</button>
                                    </td>
                                </tr>
                                ))
                            ) : (
                            <tr>
                                <td colSpan={fields.length}>데이터가 없습니다.</td>
                            </tr>
                            )}

                            {/* 신규 추가 입력행 */}
                            {newRows.map((row, index) => (
                                <tr key={`new-${index}`} className="new-row">
                                    <td></td> {/*삭제칸을 위하 */}
                                    {fields && fields.length > 0 && fields.map((f) => (
                                        <td key={f.key}>
                                        <input
                                            type={f.type}
                                            className="input-box"
                                            placeholder={f.label}
                                            value={row[f.key] || ""}
                                            onChange={(e) => handleNewChange(index, f.key, e.target.value)}
                                        />
                                        </td>
                                    ))}
                                    <td></td> {/*삭제칸을 위하 */}
                                    <td></td> {/*삭제칸을 위하 */}
                                    <td></td> {/*삭제칸을 위하 */}
                                    <td>
                                        {/*
                                        onClick={handleEdit(index)}처럼 쓰면, 렌더링 시점에 실행돼버리기 때문이에요.
                                        그래서 () => handleEdit(index) 처럼 “눌렀을 때 실행”하도록 감싸주는 거예요.
                                        */}
                                        <button style={{ color: "red" }} onClick={() => handleDeleteNewRow(index)}>삭제</button>
                                    </td>
                                    <input type="hidden" value={work.workPk} name="parentWorkPk" />
                                </tr>
                            ))}
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    );
}
